...
class EcommerceCategoryImageController(http.Controller):
    ...
    @http.route('/api/ecommerce_category/<int:category_id>/image', type='http', auth='public')
    def get_category_image(self, category_id, **kwargs):
        category = request.env['product.public.category'].sudo().browse(category_id)
        if not category or not category.exists():
            return self._get_placeholder_image()

        image_base64 = category.image_1920  # Assuming image_1920 is the field name for the image
        if not image_base64:
            return self._get_placeholder_image()

        image_data = base64.b64decode(image_base64)
        headers = [
            ('Content-Type', 'image/png'),  # Change the content type according to the image format
